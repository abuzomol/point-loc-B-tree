...
const LineSegment *pointLocationQuery(SuperNode &superRoot, Point &point) {
    ...
    {
        // get the leftSegment
        if (index < superRoot.getValSize() && superRoot.getIthLeftSemiLines(index) != nullptr) {
            leftSegment =
                    pointLocationLeft(superRoot.getIthLeftSemiLines(index), point);
        }
        //get the rightSegment from previous index if not right border
        if (0 < index < superRoot.getValSize() && superRoot.getIthRightSemiLines(index) != nullptr) {
            index--;
            rightSegment =
        ...
        {
            recursiveSegment = pointLocationQuery(*superRoot.getIthChild(index), point);
        }
    }
    return &max(*leftSegment, max(*middleSegment, max(*rightSegment, *recursiveSegment, YLeftLessThan()) , YLeftLessThan() ) , YLeftLessThan() );
}
